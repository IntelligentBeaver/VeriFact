...
class PassageLabeler:
    ...
    def __init__(self):
        ...
        self.fake_claims_passages = self._load_existing("fake_claims")
    ...
    def _load_existing(self, label_type):
        ...
        elif label_type == "fake_claims":
            file_path = FAKE_CLAIMS_LABELED_FILE
        else:
            return []
    ...
    def save_labeled_passage(self, passage, query, decision, rerank_score, faiss_score, stance=None, stance_confidence=None, from_fake_claim=False):
        ...
        Args:
            from_fake_claim: If True, also saves to fake claims labeled file
        """
        labeled_item = {
        ...
        if from_fake_claim and decision == 'relevant' and stance == 'refutes':
            ...
            with self._open_output_file(FAKE_CLAIMS_LABELED_FILE) as f:
                json.dump(self.fake_claims_passages, f, indent=2, ensure_ascii=False)
        else:
            # Normal workflow: save to appropriate category file
        ...
        for i, (passage, faiss_score, rerank_score, sapbert_score) in enumerate(zip(passages, faiss_scores_top, rerank_scores, sapbert_scores), 1):
            ...
            if auto_label == 'relevant':
                labeled = self.save_labeled_passage(
                    ...
                    from_fake_claim=True  # Mark as from fake claims workflow
                )
                session_data['labeled_passages'].append(labeled)
                labeled_count += 1
    ...
    def _collect_labels(self, subset="all"):
        ...
        if subset == "fake_claims":
            return list(self.fake_claims_passages)
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            print(f"Fake claims refutes labeled: {len(labeler.fake_claims_passages)}")
            ...
            print("Choose subset: relevant, unrelated, question, fake_claims, rel+unrel, all")
